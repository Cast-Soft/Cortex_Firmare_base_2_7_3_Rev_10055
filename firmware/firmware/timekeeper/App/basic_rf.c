...
#ifndef USE_RANDOM
	// Dedicated Timekeeper disable RX
    CC2520_RXENABLE0,   0x00,
    CC2520_RXENABLE1,   0x00,
    CC2520_FRMCTRL1,    0x00,				// SET_RXENMASK_ON_TX by default (reset value) is on
#endif

    // outputs TX exception in channel A to GPIO0 (TP35)
    CC2520_GPIOCTRL0, /*CC2520_GPIO_RX_FRM_DONE, */0x21,
...
OS_FlagID               flagRadioSPITxFrame = 0xFF;           // indicates a Tx frame has been uploaded to CC2520 by SPI
OS_FlagID               flagRadioTxDone = 0xFF;

OS_MutexID              flagSPIIODone;
volatile uint16_t       txDoneType;        // 0: successful, 1: failed
volatile uint16_t       txFIFOError = 0;   // 0: no error, 1: with error
...
void RadioInit(uint16_t panId, uint16_t shortAddr, uint8_t chan) {
  ...
  flagSPIIODone     = CoCreateFlag(1, 1); // auto-reset, flag set
  ...
  HwRadioEXTIInit();
  Send_SPI_byte(CC2520_INS_SRFOFF);        // Disable RX
...
uint32_t stxon2;
uint16_t stxon2_tim3;
uint32_t stxon3;
uint16_t stxon3_tim3;
...
void RadioTxPkt(uint16_t dstAddr, uint8_t beacon, uint8_t payloadSize, uint8_t *payload, uint8_t trigger) {
    ...
    TK_BK_REGWR8(CC2520_EXCFLAG0, ~CC2520_EXC0_TX_FRM_DONE_BM);     // TODO!!! really need to do this?
    ...
    SAVE_POINT
    diagnostic.waitSPIMachineDone = CoWaitForSingleFlag(flagSPIMachineDone, 0);
    SAVE_POINT
    if (txDoneType == 1) {      //overflow or underflow, which can stop SPI actions
        TRACE("TX error\n\r");
    }
    if (trigger) {
      extern uint8_t rx_delay;
      HwWait(/*2*/rx_delay);

      HwGPOHigh(GPO_RADIO_GPIO2);     // trigger STXON
      HwWait(2);
      HwGPOLow(GPO_RADIO_GPIO2);
    } else {
       diagnostic.waitRFBeaconSent = CoWaitForSingleFlag(flagRFBeaconSent, 0);
    }
    SAVE_POINT
    // Upload to CC2520 does not mean that the frame has been sent out into the air
    diagnostic.waitRadioTxDone = CoWaitForSingleFlag(flagRadioTxDone, 10);
    SAVE_POINT
    if (diagnostic.waitRadioTxDone != E_OK) {  // TODO!!! verify if it can happen. If not, use infinit wait
        TRACE("TX timeout error\n\r");
