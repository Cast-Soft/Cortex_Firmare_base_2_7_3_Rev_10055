    ...
    for(count = 0; count < MAXBEACONS; count++){
        if ((beacon_found[count] ) && (beacon_found[count] == pRxPktData->srcAddr)) {
            ...
            if(IMU_Dbg_Prt) {
                ...
                TRACE("%d, ", CurrBeacons[count]->seqNum);
                TRACE("-%d-, ", pRxPktData->seqNumber);
            }
            ...
            TRACE("Beacon Found slot# %d Id#:%x \n\r",count, beacon_found[count]);
            ...
            //TRACE("Location of CurrBeacons[0]:%x\n\r", &CurrBeacons[0]);
            //TRACE("Memory assignement:%x sizof Mem:%x \n\r",CurrBeacons[count],sizeof( struct BeaconData));
...
void EtherSendIMU(struct *BeaconData bData) {
    ...
    //assert(SendBeaconIndex >= 0 && SendBeaconIndex < NumBeaconsActive);
        ...
        if(bData->button_state & 0x03) != bData->LastButtonState) {
            bData->LastButtonState = (bData->button_state & 0x03);  // set them to the same for next time
            // TRACE(" Beacon:%d Button:%x\n\r",SendBeaconIndex, CurrBeacons[SendBeaconIndex]->LastButtonState);
            ...
            pIMU_Data->beaconID=bData->//beacon_found[SendBeaconIndex];    // TODO!!! add an assertion to test the range of vector
            pIMU_Data->buttonState=bData->LastButtonState;
        ...
        // if((CurrBeacons[SendBeaconIndex]-> BC_PowerMode & TK_BC_PM_IMU_ON))      // TODO !!! remove this control word. always allow
        {
            ...
            // TRACE("Sizeof BCIMUDSTRCT:%d",sizeof(struct BCIMUDSTRCT));
...

        {
            ...
            pIMUData3->version_major = RT_VERSION_MAJOR;
            pIMUData3->version_minor = RT_VERSION_MINOR;
            pIMUData3->version_revision = RT_VERSION_REVISION;
            pIMUData3->version_build = RT_VERSION_BUILD;
            ...
            // TRACE("TKID: %x Len:%x ",pIMUData3->timekeeperID,pIMUData3->contentLength);
        }
...
void RefreshDetails(void){
    ...
    for(int i32 =0;i32 < NumBeaconsActive;i32++) {
        ...
        pTKBeaconStatusDetail->beaconTimeLastActivity = pBeacon_Data->arrivalTime;
