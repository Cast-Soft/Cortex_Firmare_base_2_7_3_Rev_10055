...
void HwI2CInit(void){
 ...
 /* Default to Disable RXNE Interrupt Request */
    ...
    /* Enable and Set Interrupt Priority */
    NVIC_InitStructure.NVIC_IRQChannel = I2C_EE_IRQn; //SPI_RADIO_IMU_IRQn;
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 7//3;//2; // 2nd Highest
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
    NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
    NVIC_Init(&NVIC_InitStructure);
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 6//3;//2; // 2nd Highest
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
    NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
    NVIC_Init(&NVIC_InitStructure);   
}
...
void HwIMUEXTIInit(void) {
    ...
    //Need higher interrupt level, as it is important to synchronize clock
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 5; // chnaged from 0 to 2 0;//3; // 3rd Highest
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
    NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
    NVIC_Init(&NVIC_InitStructure);
}
