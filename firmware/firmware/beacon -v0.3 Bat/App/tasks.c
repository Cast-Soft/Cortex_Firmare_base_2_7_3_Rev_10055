...
static void check_charge(uint16_t curr_chg)
/* PRIVATE FUNCTIONS ---------------------------------------------------------*/
...
void Task3(void* pdata){
  ...
  while (1) {
    RELOAD_WATCHDOG
    SAVE_POINT
    if ((HwGPIState(GPI_SW_PWR))||(curr_chg<=BATT_TERM_PERCENT)){ //old code cutsoff at 3.5V, at what percentage is this?
      if (curr_chg<=BATT_TERM_PERCENT) {
        CoSchedLock();
#ifndef BC_HW_REVB
  ...
  CoAwakeTask(taskIMUGId);
    ...
    SAVE_POINT
    CoTickDelay(100);
    StatCheck = CoAcceptSingleFlag(btn_event_flag);
		SAVE_POINT
		RELOAD_WATCHDOG

#ifndef DISABLE_PWR_SW
		if ((HwGPIState(GPI_SW_PWR))||curr_chg<=BATT_TERM_PERCENT) {
			if (pwr_holdCount++ > 10) {
				CoSchedLock();
				HwGPOLow(GPO_5V_IMU_EN);
				HwGPOLow(GPO_RF_EN);
    ...
    if(StatCheck == E_OK){
      //check and prep button msg for txqueue if in msg mode
      //if in main msg mode
      //if(buttonstate == MAIN_MODE){
      if(btnA.message == btnA.update_msg){
        btnA.message = btnA.update_msg;
        btnA.mcount = BTN_BURST_SEND;
        msgsend=1;
      }
      if(btnB.message == btnB.update_msg){
        btnB.message = btnB.update_msg;
        btnB.mcount = BTN_BURST_SEND;
        msgsend=1;
      }
      ...
      CoClearFlag(flagBTNNewData);
      //}
      //if in ui mode
    }

    if((msgsend)&&config.radioPacketFlags & RADIOPACKET_BUTTONPRESS){
      msgsend=0; 
...
static void check_charge(uint16_t curr_chg){
  if(curr_chg > config.previous_charge){
    config.charge_capacity+= (curr_chg - config.previous_charge);
    if(config.charge_capacity>= 100){
      config.charge_count++;
      config.charge_capacity-=100;
    }
    config.previous_charge = curr_chg;
    config_update=1;    
  }
}
/*******************************************************************************
...
void Task8(void* pdata){
  ...
  while (1) {
    SAVE_POINT
    RELOAD_WATCHDOG
    CoTimeDelay(0,0,10,0);      //is called every 10 seconds, would be better to use interrupt triggering flag.
    RELOAD_WATCHDOG
    SAVE_POINT
    ...
    switch(lastBatStatus.flags){
      case 0x03:    //usb is connected and is charging
        check_charge(curr_chg);
      break;
      ...
      break;
      default:      //usb is disconnected. Battery should be draining.
        if(curr_chg < config.previous_charge){
          config.previous_charge = curr_chg;
          config_update=1;
        }
    ...
    if(task8_ticks++ ==6){
      task8_ticks=0;
      if(config.radioPacketFlags & RADIOPACKET_BATTERY){
