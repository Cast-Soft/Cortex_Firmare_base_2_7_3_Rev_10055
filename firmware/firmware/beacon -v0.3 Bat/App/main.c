...
void  main(void) {
    ...
    if(init_batmonitor(batt_designCapacity, batt_term_Voltage, batt_taperCurrent)){
        i2c_bat_cmd_read(BQ27441_STATE_OF_CHARGE_LOW, &curr_chg);
        //check current charge with the previously read charge on eeprom.
        if(curr_chg > config.previous_charge){
            //it was being charged since last use of main application
            config.charge_capacity+= (current_chg - config.previous_charge);
            if(config.charge_capacity>=100){
                config.charge_count++;
                config.charge_capacity-=100;
            }
        }
        else if(curr_chg < config.previous_charge){
            //lost charge since last use of main application
    ...
    flagBTNNewData =    CoCreateFlag(0, 0);     // Manual-reset, flag cleared
