...
#define FALSE 0
U64 eeOSTime;
extern void HwI2CInit(void);
static BOOL I2C_EE_PageWriteIMP(const uint8_t* pBuffer, uint16_t WriteAddr, uint8_t NumByteToWrite);
void EETime(){
  eeOSTime = CoGetOSTime();    
}

BOOL TIMEVALID(){
  if (CoGetOSTime() > eeOSTime + 100) {
    return FALSE;
  }
  return TRUE;
}
...
again:
  EETime();  
  if (I2C_EE_PageWriteIMP(pBuffer, WriteAddr, NumByteToWrite) == FALSE) {
    HwI2CInit();
    goto again;    
  }
}

#define TIMEOUTRETURN if (!TIMEVALID()) return FALSE
#define TIMEOUTAGAIN if (!TIMEVALID()) goto again
...
/**
  ...
  * @retval None
  */
BOOL I2C_EE_PageWriteIMP(const uint8_t* pBuffer, uint16_t WriteAddr, uint8_t NumByteToWrite)
{ 
  /* While the bus is busy */
  while(I2C_GetFlagStatus(I2C_EE, I2C_FLAG_BUSY) && TIMEVALID()); TIMEOUTRETURN;
  
  /* Send START condition */
  I2C_GenerateSTART(I2C_EE, ENABLE);
  
  /* Test on EV5 and clear it */
  while(!I2C_CheckEvent(I2C_EE, I2C_EVENT_MASTER_MODE_SELECT) && TIMEVALID()); TIMEOUTRETURN;
  
  /* Send EEPROM address for write */
  I2C_Send7bitAddress(I2C_EE, EEPROM_ADDRESS, I2C_Direction_Transmitter);

  /* Test on EV6 and clear it */
  while(!I2C_CheckEvent(I2C_EE, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED) && TIMEVALID()); TIMEOUTRETURN; 
  ...
  /* Test on EV8 and clear it */
  while(!I2C_CheckEvent(I2C_EE, I2C_EVENT_MASTER_BYTE_TRANSMITTED) && TIMEVALID()); TIMEOUTRETURN; 
