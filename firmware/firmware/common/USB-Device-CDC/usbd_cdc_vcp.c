...
#if defined(BEACON_APP) || defined(ROUTER_APP) || defined(TIMEKEEPER_APP)
#include "config.h"
#endif
...
#define APP_TX_BUFFER_SIZE      1024
...
size_t __write(int handle, const unsigned char *buffer, size_t size) {
    ...
    //Only called from printf, which is called from
    //hard fault handler only
    //no need to lock
...
/*******************************************************************************
* Description : STDOUT Low-Level write for command line response
* Input       :
* Return      :
*******************************************************************************/
size_t __writeCmdLineRespPacket(const unsigned char *buffer, size_t size, uint8_t contentType) {
  ...
  __disable_interrupt();
  do {
    if (write_header(contentType, size)) {
      break;
    }
...
{
  return __writeCmdLineRespPacket(buffer, size, DEV_RESP_IMU_DATA);
}

// intercept and add a header
char trace_format_buf[512];
int TRACE(char* fmt, ...) {
    int len;
#if defined(BEACON_APP)
    if (!(config.flags & FLAG_TRACE_ENABLE)) {
      return 0;
    }
#elif defined(ROUTER_APP) || defined(TIMEKEEPER_APP)
    if (!(config.flags & FLAG_TRACE_ENABLE)) {
        return 0;
    }
#endif
    ...
    len = vsprintf(trace_format_buf, fmt, argptr); /* print the variable arguments to buffer */
    va_end(argptr);  /* Signify end of processing of variable arguments */

#ifdef BEACON_APP
    // Send out a header

    if (config.flags & FLAG_TRACE_USE_PROTO_CMD_LINE_RESP) {
#endif
      return __writeCmdLineRespPacket((unsigned char*) trace_format_buf, len + 1, DEV_RESP_INFO);
#ifdef BEACON_APP
    } else {
