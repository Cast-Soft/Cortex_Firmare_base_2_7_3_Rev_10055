            ...
            else if (opt == OPT_DEL_ANYWAY) /* Always delete the flag         */
            {
                if(pnode->waitType == OPT_WAIT_ALL)
                {
                    ...
                    if( pnode->waitFlags == (1<<id) )
                    {
                        ...
                        TASK_SEQ(pnode->waitTask); pnode = RemoveFromLink(pnode);
                        continue;
                    }
                    else
                    {
...
StatusType CoWaitForSingleFlag(OS_FlagID id,U32 timeout)
{
    ...
    else                                /* If the required flag is not set    */
    {
        ...
        if(timeout == 0)                /* If time-out is not configured      */
        {
            ...
            curTCB->state  = TASK_WAITING; TASK_SEQ(curTCB); 
        ...
        else                            /* If time-out is configured          */
        {
            ...
            else                          /* If flag is set                   */
            {
                ...
                OsSchedLock();
                //FLAG_BUG:
                //There are rare cases when between previous OsSchedUnlock()
                //right after InsertDelayList and this OsSchedLock()
                //other task calls CoSetFlag(). As a result:
                //task is removed from delay list, but stays in TASK_WAITING
                //and is not in TCBRdy, therefore, never got called.
                //Extending locked state (remove pair Unlock/Lock did not work -
                //app stopped to work
                //Solution is to double test DlyList again after lock,
                //if we are not there, CoSetFlag was called, return as it was set
                //I assume only thing to chage would be restore previous state
                //which is TASK_RUNNING
#ifndef FLAG_BUG
                P_OSTCB dlyList = DlyList;
                U8 found = 0;
                while (dlyList) {
                  if (dlyList == curTCB) {
                    found = 1;
                  ...
                  dlyList = dlyList->TCBnext;
                }
                if (!found) {
#ifdef TASKS_PROFILE
                  curTCB->caught = __LINE__;
#endif
                  curTCB->state = oldState; TASK_SEQ(curTCB); 
                }
#endif
                /* Clear the required flag or not                             */
                pfcb->flagRdy &= ~((1<<id)&(pfcb->resetOpt));
...
static P_FLAG_NODE RemoveFromLink(P_FLAG_NODE pnode)
{
    ...
    ptcb->pnode = Co_NULL; //(void*) INVALID_VALUE; //0xffffffff;
    if(ptcb == TCBRunning)
    {

            ptcb->state = TASK_RUNNING; TASK_SEQ(ptcb); 
    }
    else
    {

            TASK_SEQ(ptcb); InsertToTCBRdyList(ptcb);         /* Insert the task to ready list        */
    }
