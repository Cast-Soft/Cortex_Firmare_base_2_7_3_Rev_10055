...
StatusType DeleteEvent(P_ECB pecb,U8 opt)
{
    ...
    else if(opt == OPT_DEL_ANYWAY)      /* Do delete event anyway?            */
    {
		...
		TASK_SEQ(ptcb); 	InsertToTCBRdyList(ptcb);         /* Insert task into ready list  */
        }
        OsSchedUnlock();                  /* Unlock schedule                  */
...
#endif
    ptcb->state = TASK_WAITING; TASK_SEQ(ptcb);    /* Set task status to TASK_WAITING state  */
...
void EventTaskToRdy(P_ECB pecb)
{
    ...
    else if(pecb->eventType == EVENT_TYPE_SEM)/* Is it a semaphore event?     */
    {
        ...
        ptcb->pmail = Co_NULL; //(void*) INVALID_VALUE; //0xffffffff;      /* Indicate task woke by event  */
    }
#endif
	if(ptcb == TCBRunning)
	{
