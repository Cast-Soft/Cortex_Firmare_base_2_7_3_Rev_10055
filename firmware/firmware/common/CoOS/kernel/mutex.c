...
StatusType CoEnterMutexSection(OS_MutexID mutexID)
{
    ...
    else if(pMutex->mutexFlag == MUTEX_OCCUPY)
    {
        ...
        if(ptcb->prio > pCurTcb->prio)  /* Need to promote priority of owner? */
        {
            ...
            {
                TASK_SEQ(ptcb); RemoveFromTCBRdyList(ptcb); /* Remove the task from READY list*/
                TASK_SEQ(ptcb); InsertToTCBRdyList(ptcb);   /* Insert the task into READY list*/
            }
#if CFG_EVENT_EN >0
        ...
        pCurTcb->state   = TASK_WAITING; TASK_SEQ(pCurTcb);  /* Block current task             */
...
void RemoveMutexList(P_OSTCB ptcb)
{
    ...
    else                      /* If the task is in the middle of waiting list */
    {
        ...
        assert(ptcb->TCBnext != ptcb->TCBprev);
