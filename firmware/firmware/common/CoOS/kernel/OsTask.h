...
#define  INVALID_VALUE  (U32)0xffffeeff /*(U32)0xffffffff*/
#define  MAGIC_WORD     (U32)0x5a5aa5a5

//#define DEBUG_OS_TASK			// This is to find out task switching issue
#define SeqSize 36
...
typedef  struct TCB
{
    ...
    U32    caught;
#ifdef DEBUG_OS_TASK
    // After remove from ready list
    U16 lastRemoveRdy; 
    // struct TCB  *TCBnextR;
    // struct TCB  *TCBprevR;   
    // U8 taskIDRA[12];             // remove after          
  
    U16 lastInsertRdy;
    // struct TCB  *TCBnextI;
    // struct TCB  *TCBprevI;
    // U8 taskListIB[12];          // insert berfore   
    // U8 taskListIA[12];          // insert after        
    
    // U16 lastTCBNext;    
    
    U8 lastAct;      // 5 Remove, 6 Insert
    //U8 actStep;      // 0 (Just removed from ready list Scheduled), 1, 2, 3, ....
...
#define TASK_SEQ(TASKCB) PushSeq(TASKCB, __LINE__)
#else
#define TASK_SEQ(TASKCB)
#endif
