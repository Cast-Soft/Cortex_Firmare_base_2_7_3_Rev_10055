        ...
        while(dlyNext != Co_NULL)
        {
            ...
            if(deltaTicks < 0)          /* Is delta ticks<0?                  */
            {
                ...
                if(dlyNext->TCBprev != Co_NULL)   /* Is head item of DELAY list? */
                {
                  ...
                  dlyNext = task4 => task3 => task8;
                  NOW: previous of task 4 is points to task 6,
                  which is task we want to insert;
                  so it ends up that taks's prev points to itself
                  */
                    /** added condition **/
                  if (dlyNext->TCBprev != ptcb) {
                    //assert(dlyNext->TCBprev != ptcb);
...
StatusType CoResetTaskDelayTick(OS_TID taskID,U32 ticks)
{
    ...
    if(ticks == 0)                        /* Is delay tick==0?                */
    {
        TASK_SEQ(ptcb); InsertToTCBRdyList(ptcb);         /* Insert task into the DELAY list  */
    }
    else
    {
...
void TimeDispose(void)
{
    ...
    while((dlyList != Co_NULL) && (dlyList->delayTick == 0) )
    {
        ...
        TASK_SEQ(dlyList); InsertToTCBRdyList(dlyList);        /* Insert task into READY list    */
