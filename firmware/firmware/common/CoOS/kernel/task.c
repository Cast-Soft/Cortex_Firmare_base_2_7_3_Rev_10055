...
#ifdef DEBUG_OS_TASK
void PushSeq(P_OSTCB task, U16 line)
{
    ++task->seqHead;
    if (task->seqHead >= SeqSize)
    {
      task->seqHead = 0;
    }
    
    task->seq[task->seqHead] = line;
}
#endif
...
void CreateTCBList(void)
{
    ...
    {
		ptcb1->taskID    = i;
		ptcb1->state     = TASK_DORMANT; TASK_SEQ(ptcb1);
                assert(ptcb1 != ptcb2);
        ptcb1->TCBnext   = ptcb2;
#if CFG_ORDER_LIST_SCHEDULE_EN ==0
...
void InsertToTCBRdyList(P_OSTCB tcbInsert)
{
    ...
    tcbInsert->state     = TASK_READY;  TASK_SEQ(tcbInsert);   /* Set task as TASK_READY             */
...
TASK_SEQ(ptcb);
    ...
    {   TASK_SEQ(ptcb);                    /* Yes,remove task from list          */
#if CFG_ORDER_LIST_SCHEDULE_EN ==0
    ...
    {  TASK_SEQ(ptcb);
		...
		if((ptcb->TCBprev->prio != prio) && (ptcb->TCBnext->prio != prio))
			...
			RdyTaskPri[seqNum] = ptcb->TCBprev->taskID;
#endif
           assert(ptcb->TCBprev->TCBnext != ptcb->TCBprev);
...
StatusType CoSetPriority(OS_TID taskID,U8 priority)
{
    ...
    {                                   /* No                                 */
        ...
        if(ptcb->state == TASK_READY)       /* Is task in READY list?         */
        {
            ...
            TASK_SEQ(ptcb); RemoveFromTCBRdyList(ptcb);
            TASK_SEQ(ptcb); InsertToTCBRdyList(ptcb);
            OsSchedUnlock();
        }
...
// line and func not inside TASKS_PROFILE
// just to simplify linking
U16             line;
const char *    func;
#ifdef TASKS_PROFILE
static OS_TID          currentTaskId;
U32             globl;
#endif
...
void Schedule(void)
{
    ...
    TaskSchedReq = Co_FALSE; TASK_SEQ(TCBNext);
    ...
    {
#ifdef DEBUG_OS_TASK
        TCBNext->lastTCBNextState = pCurTcb->state;
#endif
		TASK_SEQ(TCBNext); TCBNext        = pRdyTcb;   /* Yes,set TCBNext and reorder READY list */
        pRdyTcb->state = TASK_RUNNING; TASK_SEQ(pRdyTcb);
        TASK_SEQ(pRdyTcb); RemoveFromTCBRdyList(pRdyTcb); TASK_SEQ(pRdyTcb);
    }

    else if(RdyPrio < RunPrio )     /* Is higher PRI task coming in?          */
    {
        TASK_SEQ(TCBNext); TCBNext        = pRdyTcb;   /* Yes,set TCBNext and reorder READY list */
        TASK_SEQ(pCurTcb); InsertToTCBRdyList(pCurTcb);
	TASK_SEQ(pRdyTcb); RemoveFromTCBRdyList(pRdyTcb); TASK_SEQ(pRdyTcb);  
        TASK_SEQ(pRdyTcb); pRdyTcb->state = TASK_RUNNING;
    }
    ...
    TASK_SEQ(TCBNext);
#ifdef TASKS_PROFILE
    TCBRunning->func = func;
    TCBRunning->line = line;
    TCBRunning->glbFirst = TCBRunning->glbLast;
    TCBRunning->glbLast = globl;
    TCBRunning->scheduled++;
    func = TCBNext->func;
    line = TCBNext->line;
    currentTaskId = TCBNext->taskID;
    globl++;
#endif
        TASK_SEQ(TCBNext); SwitchContext(); TASK_SEQ(TCBNext);                             /* Call task context switch */
}

#ifdef TASKS_PROFILE

OS_TID CoGetCurrentTaskId()
{
  return currentTaskId;
}

U32 CoGetStackDepth(OS_TID taskID)
{
      P_OSTCB ptcb;
  ...
  return ptcb->sktSz - (ptcb->stack - ptcb->stkPtr);

}

U32 CoGetTaskScheduledCount(OS_TID taskID)
{
      P_OSTCB ptcb;
  ...
  return ptcb->scheduled;
}

U16 CoGetTaskLine(OS_TID taskID) {
      P_OSTCB ptcb;
...
const char* CoGetTaskFunc(OS_TID taskID) {
      P_OSTCB ptcb;
...
OS_TID CreateTask(FUNCPtr task,void *argv,U32 parameter,OS_STK *stk)
{
...
#ifdef TASKS_PROFILE
    U16 stackSz = (parameter&0xfff00)>>8;
    ptcb->sktSz = stackSz;
    ptcb->scheduled = 0;
    ptcb->func = "None";
    ptcb->glbFirst = 0;
    ...
    *(U32*)(ptcb->stack) = MAGIC_WORD;
	...
	{									/* No,set it into ready list          */
		OsSchedLock();                  /* Lock schedule                      */
		TASK_SEQ(ptcb); InsertToTCBRdyList(ptcb);       /* Insert into the READY list         */
	    OsSchedUnlock();                /* Unlock schedule                    */
	}
	else
	{									/* Yes,Set task status as TASK_WAITING*/
		ptcb->state   = TASK_WAITING; TASK_SEQ(ptcb);
	}
...
StatusType CoDelTask(OS_TID taskID)
{
    ...
    if(ptcb->state == TASK_READY)       /* Is task in READY list?             */
    {
        TASK_SEQ(ptcb); RemoveFromTCBRdyList(ptcb);     /* Yes,remove task from the READY list*/
...
#endif
    ptcb->state   = TASK_DORMANT; TASK_SEQ(ptcb);   /* Release TCB                        */
	TaskSchedReq  = Co_TRUE;
...
StatusType CoSuspendTask(OS_TID taskID)
{
    ...
    {
        TASK_SEQ(ptcb); RemoveFromTCBRdyList(ptcb);     /* No,Remove task from READY list     */
    }
	else
	{
		TaskSchedReq = Co_TRUE;
    ...
    ptcb->state = TASK_WAITING;	TASK_SEQ(ptcb);        /* Set task status as TASK_WAITING    */
...
StatusType CoAwakeTask(OS_TID taskID)
{
    ...
    OsSchedLock();                      /* Lock schedule                      */
	TASK_SEQ(ptcb); InsertToTCBRdyList(ptcb);           /* Insert the task into the READY list*/
    OsSchedUnlock();                    /* Unlock schedule                    */
    return E_OK;                        /* return OK                          */
}
#endif
