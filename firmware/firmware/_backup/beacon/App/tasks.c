...
void Task3(void* pdata){
    ...
    while (1) {
      RELOAD_WATCHDOG
...
#if 0	//this is done prior to multitask as the IMU requires instant settings for SPI mode
        // Init Imu
        // Not sure why but I need delay in between IMU reg access.
        // THis slows the bootup a bit but seems to work for now ==> IMU SPI critical section issue -- Albert
        //CoTickDelay(10);
        static uint8_t temp = 0;


        // Accel ODR = 200 hz, enable all axis
        IMURegWr(IMU_CTRL_REG1, SPI_A_IMU, (IMU_A_RATE_200HZ | IMU_A_AXIS_X_EN |
                                            IMU_A_AXIS_Y_EN | IMU_A_AXIS_Z_EN));
        // Gyro ODR = 190Hz
        IMURegWr(IMU_CTRL_REG1, SPI_G_IMU, 0x7F);
        // Gyro DRDY pin enabled
        IMURegWr(IMU_CTRL_REG3, SPI_G_IMU, 0x08);

        // Reverse default endianess to big endian same as AD
        // accel high res mode, 2G scale
        IMURegWr(IMU_CTRL_REG4, SPI_A_IMU, IMU_BIG_ENDIAN | IMU_H_RESOL);
        IMURegWr(IMU_CTRL_REG4, SPI_G_IMU, IMU_BIG_ENDIAN);

        temp = IMURegRd(IMU_WHOAMI_REG, SPI_G_IMU);
        if(temp == IMU_IM_LSM330)
        {
          IMUPresent=1;
        }
        else
        ...
        assert(temp == IMU_IM_LSM330);

        // Quickly do a read of the data to ensure that the data reg is empty or
        // we run the risk of missing the DRDY event
        CoClearFlag(flagIMUNewData);
        
        HwSPISSAssert(SPI_G_IMU);
        DMA_Cmd(SPI_IMU_RX_DMA_CHAN, DISABLE);
        DMA_Cmd(SPI_IMU_TX_DMA_CHAN, DISABLE);
        SPI_IMU_RX_DMA_CHAN->CNDTR = 4;
        SPI_IMU_TX_DMA_CHAN->CNDTR = 4;
        ...
        // Here is a hack for changing IMU_G task priority to allow
        // read data and set it back later
        //CoSetPriority(taskIMUGId, 0);
        //yield
        //Schedule();
...
//((config.radioPacketFlags & RADIOPACKET_BATTERY) && (battery_minutes != (secs/600))
      if ((config.flags & FLAG_SEND_BATTERY_INFO) && sec > last_bat_sent) {
        last_bat_sent = 10 + (random%40);
        if (config.flags & FLAG_TRACE_ASYNC) {
          uint8_t hours = sec/3600;
          uint8_t min = (sec/60)%60;
          uint8_t mysec = (sec%60);

          TRACE("Battery: %d.%04dV charge=%u%% %02d:%02d:%02d  %s charging; next in %d sec\n\r",
              (uint8_t)fVolt, ((uint16_t) (fVolt*10000.0))%10000, last_percents,
              hours, min, mysec,
              lastBatStatus.flags == 3?"":"not", last_bat_sent);

        }
        last_bat_sent += sec;
        static uint8_t ticks = 0;
        lastBatStatus.tick = ticks++;
        lastBatStatus.crc8 = crc8(&lastBatStatus.type, sizeof(lastBatStatus) - 1);
        SAVE_POINT
        RadioTxPktQueue(routerAddr, sizeof(lastBatStatus), (uint8_t*) &lastBatStatus);
        SAVE_POINT
      }
    //  fVolt = 0.0;
...
void TaskIMU_G(void* pdata){
  while(1)
  {
    ...
    counterIMU++;
    // Removed because handled by tasksWDT
    SAVE_POINT
    ...
    CoWaitForSingleFlag(flagIMUNewData, 0);
    SAVE_POINT
    // Accel Data is ready. Now move to buffer
    //InputDataIntoBuffer(&ImuAccelBuffer, &spiIMURxBufDma[1]);
    // Gyro Data is ready. Now move to buffer
    InputDataIntoBuffer(&ImuGyroBuffer, &spiIMURxBufDma[1]);
    ...
    //test_imu_pkt_ctr++;  // increment IMU packet counter
    setIMUReady = CoSetFlag(flagIMUDataReady);
    SAVE_POINT
    // Check to see if we missed the next DRDY event
    if(HwGPIState(GPI_IMU_DIO1))
    {
      EXTI_GenerateSWInterrupt(GPI_IMU_DIO1_EXTI_LINE);
    }
#endif
	//REV J 
	RadioIMU_WaitGrabSPI();
