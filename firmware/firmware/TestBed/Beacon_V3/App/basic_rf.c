...
volatile uint16_t   flagRadioTxAllow= 0;
#endif
...
#endif
#ifdef BC_HW_REVC
           HwGPOHigh(GPO_RF_EN);  // enable the pwer supply
            HwWait(10);
          HwGPOLow(GPO_2520_RST);  // take rest low
            HwWait(10);
          HwGPOHigh(GPO_2520_RST);  // rest high
#endif
...
/* Load defalt Radio enable values   */
    ...
    if (config.TestMode){
     EXTI->IMR &= ~GPI_IMU_DIO1_EXTI_LINE; // imu off
//      RxTx_Setup();
       RadioTxTest();
       
 //       Send_SPI_2byte(CC2520_INS_TXBUF, 0x78);
 //       TK_BK_REGWR8(CC2520_FRMCTRL0,0x03);
 //      Send_SPI_byte(CC2520_INS_STXON);        // enable TX  
    ...
    TK_BK_REGWR8(CC2520_GPIOCTRL2, 0x80 | CC2520_STR_STXON); // **note**: Not STXONCCA
...
void RadioTxPkt(uint16_t dstAddr, uint8_t beacon, uint8_t payloadSize, uint8_t *payload, uint8_t trigger) {
...
#ifdef COOS
    static uint16_t entrance_count = 0; // [[DEBUG]]
    static uint16_t radioTxCount = 0; // [[DEBUG]]
    ...
    while (1) {
        // attempt to grab SPI resource
        RadioWaitGrabSPI();

        if (txInhibit) {
            // SPI will be needed to RX the RF Sync Frame
            RadioReleaseSPI();
            // wait till we are allowed to TX (i.e. not within TX Inhibit guard-band)
#ifdef COOS
            CoWaitForSingleFlag(flagRadioTxAllow, 0);
#else
            while (!flagRadioTxAllow);
#endif
            // now we've to start again because we gave up SPI
        } else {
            break;
        }
    }
    ...
    // unfortunately necessary because CCA is slow to assert after TX
    while (!(TK_BK_REGRD8(CC2520_FSMSTAT1) & CC2520_FSMSTAT1_CCA_BM));
...
int8_t status;
     status = Send_SPI_byte(CC2520_INS_SNOP); // save curent status 
      Send_SPI_byte(CC2520_INS_SRFOFF);
      TK_BK_MEMWR8(CC2520_TXPOWER,TxLev);
         Send_SPI_byte(CC2520_INS_SNOP);
        Send_SPI_byte(CC2520_INS_SNOP);
