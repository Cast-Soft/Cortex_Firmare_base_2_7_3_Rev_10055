...
static OS_STK taskLED_Stack[OS_STACK_SIZE];
static OS_STK taskUI_Stack[OS_STACK_SIZE];
/* PUBLIC VARIABLES ----------------------------------------------------------*/
...
OS_FlagID flagLEDSync;
OS_FlagID flagEInk_RDY;
OS_FlagID flagEInk_DMA_Done;
OS_FlagID flagBtn_UI;
OS_FlagID flagLED_UI;
OS_FlagID flagTimeout_UI;
OS_FlagID flagUSBdata_UI;
...
void  main(void) {
    ...
    semInkUI = CoCreateSem(1,1,EVENT_SORT_TYPE_PRIO); //buttons are more important than stringer indications
    assert(semInkUI != E_CREATE_FAIL);
    ...
    flagIMUDataReady    = CoCreateFlag(1, 0); 	// auto-reset, flag clear
    flagLEDSync         = CoCreateFlag(0, 0);   // Manual-reset, flag clear
    flagEInk_RDY        = CoCreateFlag(1, 0);   // auto-reset, flag clear
    flagEInk_DMA_Done   = CoCreateFlag(0, 0);   // manual-reset, flag clear
    flagBtn_UI          = CoCreateFlag(0, 0);   //manual-reset, flag clear
    flagLED_UI          = CoCreateFlag(0, 0);   //manual-reset, flag clear
    flagTimeout_UI      = CoCreateFlag(0, 0);   //manual-reset flag clear
    flagUSBdata_UI      = CoCreateFlag(0, 0);   //manual-reset flag clear
    flagIMU_read        = CoCreateFlag(1, 0);
    ...
    // IRLed detection task
    taskLEDId       = CoCreateTaskEx(TaskLED, (void*)0,6, &taskLED_Stack[OS_STACK_SIZE-1], OS_STACK_SIZE, (uint32_t)0, (uint32_t)1);

    // UI state machine task
    taskUIId        = CoCreateTaskEx(TaskUI, (void*)0,8, &taskUI_Stack[OS_STACK_SIZE-1], OS_STACK_SIZE, (uint32_t)0, (uint32_t)1);
