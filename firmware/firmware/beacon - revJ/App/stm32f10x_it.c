...
/*
    NVIC Priority (Highest first):
        - Priority 0: OTG_FS_IRQ
        - Priority 0: EXTI2_IRQn
        - Priority 1: TIM2_IRQn
        - Priority 2: TIM3_IRQn
        - Priority 3: SPI3_IRQn (SPI_RADIO_IRQn)
        - Priority 4: EXTI2_IRQn (GPI_RADIO_GPIO0_IRQn)
        - Priority 4: EXTI1_IRQn (GPI_RADIO_GPIO1_IRQn)
        - Priority 4: EXTI3_IRQn (GPI_RADIO_GPIO5_IRQn)

        - Priority 6: EXTI10_15_IRQn (GPI_IMU_INT_IRQn)
        - Priority 6: TIM6_IRQn (NOT USED)
        - Priority 8: TIM5_IRQn (NOT USED -- USED ONLY IF CCA_EN ENABLED)
        - PRIORITY 9: TIM1_IRQn
*/
// EXPERIMENT I2C EVENT/ERROR INTERRUPTS ON PRIORITY 5 OR 7
...
volatile int frameIdInced = 0;
// =======================  Key synchronization variables. Must carefully guard operations on them ==================================
volatile uint32_t frameId = 0;           // together with TIM3->CCR1 determines beacons' frame id and phase(fraction of a frame). So make sure that operations on frameeId and TIM3->CCR1 are atomic.
                                                // also, if frameId changed and TIM3->CCR1 got reloaded, frameId must be adjusted accordingly
// These are a pair. Must be atomially used. local frame Id. TODO! rename them
volatile uint32_t frameIdAtSync;        // when detecting a packet is arriving in EXTI2_IRQHandler, beacon assumes that it is from T.K but it is until entire packet is received and processed by task TaskRadioRx() 
                                        // that beacon knows the packet is real or just noise. So if it is noise. Then they are useless as TaskRadioRx() will not be triggered
volatile uint16_t tim3PhaseAtSync;      // frameIdAtSync and tim3PhaseAtSync must be capture at the same time atomically
                                        // if just after frameIdAtSync captured, tim3PhaseAtSync got reloaded, then both frameId frameIdAtSync and tim3PhaseAtSync has to re-achieve again 
//====================================================================================================================================
...
void hard_fault_handler_c(unsigned int * hardfault_args)
{
  ...
  I2C_ITConfig(I2C_EE, (I2C_IT_EVT | I2C_IT_BUF), DISABLE); 
...
//      }
    ...
    if (TIM_GetITStatus(TIM3, TIM_IT_CC2) == SET)
    {
       ...
       // TIM1->CNT = 0;
        ...
        switch(led_select){
          case select_IRLED0:
            if(HwGPOstatus(GPO_IRLED0)){
              ENTER_ISR();
              ISR_SETFLAG(flagLEDSync);
              EXIT_ISR();
            }
            break;
          case select_IRLED1:
            if(HwGPOstatus(GPO_IRLED1)){
              ENTER_ISR();
              ISR_SETFLAG(flagLEDSync);
              EXIT_ISR();
            }
...
void EXTI2_IRQHandler(void) {
  ...
  if (frameIdFlag) {
    ...
    tim3PhaseAtSync = TIM3->CNT;        // TODO! existing abs(tim3PhaseAtSync - TIM3_AUTORELOAD) <= delta &&  TIM3->CNT > delta then it could be noise                               
    // if it is just turning to 0 and TIM_IT_Update got set EXTI2 has higest priority and TIM3 will not be triggered. Both TIM3 and tasks should treat frameIdAtSync and tim3PhaseAtSync in critical section 
    if (TIM_GetITStatus(TIM3, TIM_IT_Update) == SET) {
        frameIdAtSync++;
        tim3PhaseAtSync = TIM3->CNT;            // should be value as ARR. Need to verify. not 0! TODO!
        frameIdInced = 1;
      } else {
        frameIdInced = 0;
      }    
...
void DMA1_Channel4_IRQHandler(void)
{
    ...
    if(DMA_GetITStatus(DMA1_IT_TC4)) {
        // Clear DMA1 Channel5 Transfer Complete interrupt pending bit
        DMA_ClearITPendingBit(DMA1_IT_TC4);
        HwSPISSDeAssert(SPI_INK);
        ISR_SETFLAG(flagEInk_DMA_Done);
    } else
    assert(0);
    EXIT_ISR();
}
...
/*******************************************************************************
* Description : This function handles I2Cx Error interrupt request.
* Priority    : 6
* Input       : -
* Return      : -
* must include EE_gettime? like in i2c_ee
*******************************************************************************/
void I2C1_ER_IRQHandler(void){ 
  ENTER_ISR();
  if(I2C_GetITStatus(I2C_EE, I2C_IT_SMBALERT)){
		I2C_ClearITPendingBit(I2C_EE, I2C_IT_SMBALERT);
	}
	if(I2C_GetITStatus(I2C_EE, I2C_IT_TIMEOUT)){
		I2C_ClearITPendingBit(I2C_EE, I2C_IT_TIMEOUT);
	}
	if(I2C_GetITStatus(I2C_EE,I2C_IT_PECERR)){
		I2C_ClearITPendingBit(I2C_EE, I2C_IT_PECERR);
	}
	if(I2C_GetITStatus(I2C_EE,I2C_IT_OVR)){
		I2C_ClearITPendingBit(I2C_EE, I2C_IT_OVR);
	}
	if (I2C_GetITStatus(I2C_EE, I2C_IT_AF)){
		I2C_ClearITPendingBit(I2C_EE, I2C_IT_AF);
	}
	if(I2C_GetITStatus(I2C_EE, I2C_IT_ARLO)){
		I2C_ClearITPendingBit(I2C_EE, I2C_IT_ARLO);
	}
	if(I2C_GetITStatus(I2C_EE, I2C_IT_BERR)){
		I2C_ClearITPendingBit(I2C_EE, I2C_IT_BERR);
	}
  I2C_ERR=1;
  ISR_SETFLAG(flagI2CMachineDone);
...
void I2C1_EV_IRQHandler(void){
  ...
  ENTER_ISR();
  check = I2C_GetLastEvent(I2C_EE);
  switch(check){
    case I2C_EVENT_MASTER_MODE_SELECT:                                    //EV5
    //check write or read mode.
      if(I2CTxRxByteState == I2C_READ_MODE){
        I2C_Send7bitAddress(I2C_EE, *pI2CTxBuf, I2C_Direction_Receiver);
      }
      else{ // if(I2CTxRxByteState == I2C_WRITE_MODE){
        I2CTxRxByteCount--;
        I2C_Send7bitAddress(I2C_EE, *(pI2CTxBuf++), I2C_Direction_Transmitter);
      }
      break;
/* Master Receiver ------------------------------------------------------------*/
    ...
    case (I2C_EVENT_MASTER_BYTE_RECEIVED | 0x04):
  case (0x40):
      *(pI2CRxBuf++) = I2C_ReceiveData(I2C_EE);
      I2CTxRxByteCount--;
      if(I2CTxRxByteCount==1){
        I2CTxRxByteState = I2C_INIT_MODE;
        I2C_AcknowledgeConfig(I2C_EE, DISABLE);
        I2C_GenerateSTOP(I2C_EE, ENABLE);
      }
      else if(I2CTxRxByteCount==0){
        I2CTxRxByteState = I2C_INIT_MODE;
        ISR_SETFLAG(flagI2CMachineDone);
        I2C_AcknowledgeConfig(I2C_EE, ENABLE);
        I2C_ITConfig(I2C_EE, (I2C_IT_EVT|I2C_IT_ERR|I2C_IT_BUF), DISABLE);
...
/* Master Transmitter --------------------------------------------------------*/
    case I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED:                      //EV6
      if(I2CTxRxByteCount--){
        I2C_SendData(I2C_EE, *(pI2CTxBuf++));        
      }
      if(I2CTxRxByteCount==0){
        I2C_ITConfig(I2C_EE, I2C_IT_BUF, DISABLE);
      }
      break;
    ...
    case I2C_EVENT_MASTER_BYTE_TRANSMITTED:                               //EV8_2
      ...
      break;
    default:
      //something wrong has occurred. undefined state ->something to look into if this occurs as it shouldn't occur
      //assert(0);
      I2C_ERR=1;
      ISR_SETFLAG(flagI2CMachineDone);
      I2C_ITConfig(I2C_EE, I2C_IT_EVT|I2C_IT_ERR|I2C_IT_BUF, DISABLE);
...
/*******************************************************************************
* Description : This function handles TIM7 Update interrupt request.
* Priority    : 10
* Input       : -
* Return      : -
*******************************************************************************/
